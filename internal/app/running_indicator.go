...
func newRunningQueryContext(label string, startedAt time.Time) *RunningQueryContext {
	if startedAt.IsZero() {
		startedAt = time.Now()
	}

	return &RunningQueryContext{
		Label:     strings.TrimSpace(label),
		StartedAt: startedAt,
	}
}

func formatRunningIndicator(running *RunningQueryContext) string {
	if running == nil {
		return ""
	}
