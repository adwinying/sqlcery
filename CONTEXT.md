...
### Connection Picker
A single-step selector for choosing which named Connection to open. It is one Modal with one shared selection state (the **Connection Picker Context**), presented the same way in both situations it appears: as a centered overlay box over the panes, whether at startup (launched without a connection argument, no Session yet — the panes behind it are empty) or when switching Connections mid-run (panes hold the live Session). The two situations differ only in behaviour, not appearance: at startup an empty-filter Esc **quits** SQLcery and an empty `connections.toml` adds a "create it and restart" hint, because there is nothing to fall back to; mid-run an empty-filter Esc **closes** the Picker non-destructively, returning to the live Session. Selecting a Connection keeps the Picker visible while connecting; on failure the failed Connection is marked inline and the error detail is shown in the Status Bar. Lists only named Connections from `connections.toml`, ordered by Connection Frecency, with a fuzzy filter over names. Distinct from a Wizard — the Picker is a single pick, not a guided multi-step flow. Creating a *new* Connection is a separate, not-yet-built flow (the New Connection Wizard).
